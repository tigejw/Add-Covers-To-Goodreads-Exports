...
async function fetchCovers(bookID) {
  try {
    ...
    //this might break if goodreads ever changes site/html layout:3
    const cover =
...
async function fetchBookCovers(books) {
  ...
  for (const book of books) {
    const cover = await fetchCovers(book["Book Id"]);
    if (cover) {
      progress ++;
      console.log(`Fetched cover ${progress} of ${books.length}: ${cover}`);
      book.cover_url = cover;
    } else {
      console.log(`Failed to fetch cover ${progress} for:`, book["Book Id"]);
    }
  }
  return books;
}
